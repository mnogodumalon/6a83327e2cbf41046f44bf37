...
export function MitgliederDetails({
  ...
  return (
    <>
      ...
      <SatelliteSection
        title={appLabel('sitzungen')}
        items={sitzungenList.filter(r => Array.isArray(r.fields.angemeldete_mitglieder) && r.fields.angemeldete_mitglieder.some((u: unknown) => extractRecordId(u) === record.record_id))}
        map={r => ({ name: r.fields.titel ?? appLabel('sitzungen'), meta: r.fields.datum_uhrzeit })}
        onOpen={onOpenSitzungen}
        onAdd={onAddSitzungen}
        getKey={r => r.record_id}
      ...
      <SatelliteSection
        title={appLabel('protokolle')}
        items={protokolleList.filter(r => Array.isArray(r.fields.anwesende_mitglieder) && r.fields.anwesende_mitglieder.some((u: unknown) => extractRecordId(u) === record.record_id))}
        map={r => ({ name: appLabel('protokolle'), meta: r.fields.erstellungsdatum })}
        onOpen={onOpenProtokolle}
        onAdd={onAddProtokolle}
        getKey={r => r.record_id}
