...
export interface MitgliederDetailsProps {
  ...
  sitzungenList2: Sitzungen[];
  /** Zeilen-Klick → overlay.push auf das Sitzungen-Detail (nie der Edit-Dialog). */
  onOpenSitzungen2: (record: Sitzungen) => void;
  /** Kontextuelles „+": öffnet den Sitzungen-Dialog mit diesem Record vorgesetzt. */
  onAddSitzungen2: () => void;
  ...
  protokolleList2: Protokolle[];
  /** Zeilen-Klick → overlay.push auf das Protokolle-Detail (nie der Edit-Dialog). */
  onOpenProtokolle2: (record: Protokolle) => void;
  /** Kontextuelles „+": öffnet den Protokolle-Dialog mit diesem Record vorgesetzt. */
  onAddProtokolle2: () => void;
  ...
  notizenList2: Notizen[];
  /** Zeilen-Klick → overlay.push auf das Notizen-Detail (nie der Edit-Dialog). */
  onOpenNotizen2: (record: Notizen) => void;
  /** Kontextuelles „+": öffnet den Notizen-Dialog mit diesem Record vorgesetzt. */
  onAddNotizen2: () => void;
}
...
export function MitgliederDetails({
  ...
  return (
    <>
      ...
      <SatelliteSection
        title={appLabel('sitzungen')}
        items={sitzungenList2.filter(r => Array.isArray(r.fields.angemeldete_mitglieder) && r.fields.angemeldete_mitglieder.some((u: unknown) => extractRecordId(u) === record.record_id))}
        map={r => ({ name: r.fields.titel ?? appLabel('sitzungen'), meta: r.fields.datum_uhrzeit })}
        onOpen={onOpenSitzungen2}
        onAdd={onAddSitzungen2}
        getKey={r => r.record_id}
      />
      ...
      <SatelliteSection
        title={appLabel('protokolle')}
        items={protokolleList2.filter(r => Array.isArray(r.fields.anwesende_mitglieder) && r.fields.anwesende_mitglieder.some((u: unknown) => extractRecordId(u) === record.record_id))}
        map={r => ({ name: appLabel('protokolle'), meta: r.fields.erstellungsdatum })}
        onOpen={onOpenProtokolle2}
        onAdd={onAddProtokolle2}
        getKey={r => r.record_id}
      />
      ...
      <SatelliteSection
        title={appLabel('notizen')}
        items={notizenList2.filter(r => extractRecordId(r.fields.mitglied) === record.record_id)}
        map={r => ({ name: r.fields.titel ?? appLabel('notizen'), meta: r.fields.datum })}
        onOpen={onOpenNotizen2}
        onAdd={onAddNotizen2}
        getKey={r => r.record_id}
      />
