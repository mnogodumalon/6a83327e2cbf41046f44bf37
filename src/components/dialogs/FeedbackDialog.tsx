/**
 * FeedbackDialog — pre-generated create/edit dialog for Feedback.
 *
 ...
 * FeedbackDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Feedback['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<FeedbackDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Feedback, Tagesordnungspunkte, Mitglieder, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Feedback';
...
/** Widened prefill type for FeedbackDialog.defaultValues — see file header. */
export type FeedbackDialogDefaults = Omit<Feedback['fields'], 'kategorie' | 'bewertung'> & {
    kategorie?: LookupValue | string;
    bewertung?: LookupValue | string;
  };

interface FeedbackDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Feedback['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: FeedbackDialogDefaults;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  kategorie: LOOKUP_OPTIONS['feedback']?.['kategorie'] ?? [],
  bewertung: LOOKUP_OPTIONS['feedback']?.['bewertung'] ?? [],
};
...
export function FeedbackDialog({ open, onClose, onSubmit, defaultValues, recordId, tagesordnungspunkteList, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: FeedbackDialogProps) {
  const [fields, setFields] = useState<Partial<Feedback['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['tagesordnungspunkt'] as const;
  ...
  }), [tagesordnungspunkteList, mitgliederList, ]);
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Feedback['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'feedback');
      await onSubmit(clean as Feedback['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "tagesordnungspunkt": string | null, // Display name from Tagesordnungspunkte (see <available-records>)\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n  "kategorie": LookupValue | null, // Kategorie (select one key: "zustimmung" | "ablehnung" | "enthaltung" | "anmerkung") mapping: zustimmung=Zustimmung, ablehnung=Ablehnung, enthaltung=Enthaltung, anmerkung=Anmerkung\n  "bewertung": LookupValue | null, // Bewertung (select one key: "bewertung_1" | "bewertung_2" | "bewertung_3" | "bewertung_4" | "bewertung_5") mapping: bewertung_1=1 – Sehr gut, bewertung_2=2 – Gut, bewertung_3=3 – Befriedigend, bewertung_4=4 – Ausreichend, bewertung_5=5 – Mangelhaft\n  "kommentar": string | null, // Kommentar / Feedback\n  "datum": string | null, // YYYY-MM-DD\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["tagesordnungspunkt", "mitglied"]);
        ...
        return merged as Partial<Feedback['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('feedback') })
    : t('new_entity', { entity: appLabel('feedback') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'tagesordnungspunkt': (
      <div key="tagesordnungspunkt" className="space-y-1.5">
        <Label htmlFor="tagesordnungspunkt">{fieldLabel('feedback', 'tagesordnungspunkt')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="tagesordnungspunkt"
          placeholder="Zu welchem TOP das Feedback?"
        ...
        />
        {showErrors && !fields.tagesordnungspunkt && (
    ...
    'mitglied': (
      <div key="mitglied" className="space-y-1.5">
        <Label htmlFor="mitglied">{fieldLabel('feedback', 'mitglied')}</Label>
        <Combobox
          id="mitglied"
          placeholder="Von welchem Mitglied?"
    ...
    'kategorie': (
      <div key="kategorie" className="space-y-1.5">
        <Label htmlFor="kategorie">{fieldLabel('feedback', 'kategorie')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'zustimmung'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'zustimmung' ? undefined : 'zustimmung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'zustimmung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'kategorie', 'zustimmung') ?? 'Zustimmung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'ablehnung'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'ablehnung' ? undefined : 'ablehnung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'ablehnung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'kategorie', 'ablehnung') ?? 'Ablehnung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'enthaltung'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'enthaltung' ? undefined : 'enthaltung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'enthaltung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'kategorie', 'enthaltung') ?? 'Enthaltung'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.kategorie) === 'anmerkung'}
            onClick={() => setFields(f => ({ ...f, kategorie: (lookupKey(f.kategorie) === 'anmerkung' ? undefined : 'anmerkung') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.kategorie) === 'anmerkung'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'kategorie', 'anmerkung') ?? 'Anmerkung'}
          </button>
        </div>
      </div>
    ),
    'bewertung': (
      <div key="bewertung" className="space-y-1.5">
        <Label htmlFor="bewertung">{fieldLabel('feedback', 'bewertung')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bewertung) === 'bewertung_1'}
            onClick={() => setFields(f => ({ ...f, bewertung: (lookupKey(f.bewertung) === 'bewertung_1' ? undefined : 'bewertung_1') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bewertung) === 'bewertung_1'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'bewertung', 'bewertung_1') ?? '1 – Sehr gut'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bewertung) === 'bewertung_2'}
            onClick={() => setFields(f => ({ ...f, bewertung: (lookupKey(f.bewertung) === 'bewertung_2' ? undefined : 'bewertung_2') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bewertung) === 'bewertung_2'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'bewertung', 'bewertung_2') ?? '2 – Gut'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bewertung) === 'bewertung_3'}
            onClick={() => setFields(f => ({ ...f, bewertung: (lookupKey(f.bewertung) === 'bewertung_3' ? undefined : 'bewertung_3') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bewertung) === 'bewertung_3'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'bewertung', 'bewertung_3') ?? '3 – Befriedigend'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bewertung) === 'bewertung_4'}
            onClick={() => setFields(f => ({ ...f, bewertung: (lookupKey(f.bewertung) === 'bewertung_4' ? undefined : 'bewertung_4') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bewertung) === 'bewertung_4'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'bewertung', 'bewertung_4') ?? '4 – Ausreichend'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.bewertung) === 'bewertung_5'}
            onClick={() => setFields(f => ({ ...f, bewertung: (lookupKey(f.bewertung) === 'bewertung_5' ? undefined : 'bewertung_5') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.bewertung) === 'bewertung_5'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('feedback', 'bewertung', 'bewertung_5') ?? '5 – Mangelhaft'}
          </button>
        </div>
      </div>
    ),
    'kommentar': (
      <div key="kommentar" className="space-y-1.5">
        <Label htmlFor="kommentar">{fieldLabel('feedback', 'kommentar')}</Label>
        <Textarea
          id="kommentar"
          placeholder="Weitere Anmerkungen, Begründung, Vorschläge..."
          value={fields.kommentar ?? ''}
          onChange={e => setFields(f => ({ ...f, kommentar: e.target.value }))}
          rows={3}
        />
      </div>
    ),
    'datum': (
      <div key="datum" className="space-y-1.5">
        <Label htmlFor="datum">{fieldLabel('feedback', 'datum')}</Label>
        <DatePicker
          id="datum"
          placeholder="Wann wurde das Feedback gegeben?"
          mode="date"
  ...
  const FIELD_LABELS: Record<string, string> = {"tagesordnungspunkt": "Tagesordnungspunkt", "mitglied": "Mitglied", "kategorie": "Kategorie", "bewertung": "Bewertung", "kommentar": "Kommentar / Feedback", "datum": "Datum des Feedbacks"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"tagesordnungspunkt": {"sitzung": "Sitzung", "punkt_titel": "Titel des Tagesordnungspunkts", "beschreibung": "Beschreibung", "reihenfolge": "Reihenfolge / Nummer", "dauer": "Geplante Dauer (Minuten)", "typ": "Typ des Punktes", "referent": "Referent / Verantwortliche Person", "unterlagen": "Unterlagen / Anhänge"}, "mitglied": {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "funktion": "Funktion im Gremium", "abteilung": "Abteilung / Organisation", "eintrittsdatum": "Eintrittsdatum", "status": "Status", "profilbild": "Profilbild", "anmerkungen": "Anmerkungen"}};
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.FEEDBACK} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
