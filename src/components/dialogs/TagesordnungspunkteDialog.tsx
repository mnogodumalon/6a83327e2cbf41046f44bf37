/**
 * TagesordnungspunkteDialog — pre-generated create/edit dialog for Tagesordnungspunkte.
 *
 ...
 * TagesordnungspunkteDialogDefaults — NOT the entity field type: lookup fields accept
 * the bare KEY string (or LookupValue), applookup fields the bare record id
 * (or record URL); the dialog normalizes. Type prefill STATE with the export:
 *  ❌ useState<Partial<Tagesordnungspunkte['fields']>>({ … })   // LookupValue fields reject string prefills (TS2322)
 *  ✓ useState<TagesordnungspunkteDialogDefaults | undefined>(undefined)
 */
import { useState, useEffect, useMemo, useRef, useCallback } from 'react';
import type { Tagesordnungspunkte, Sitzungen, Mitglieder, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Tagesordnungspunkte';
...
/** Widened prefill type for TagesordnungspunkteDialog.defaultValues — see file header. */
export type TagesordnungspunkteDialogDefaults = Omit<Tagesordnungspunkte['fields'], 'typ'> & {
    typ?: LookupValue | string;
  };

interface TagesordnungspunkteDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Tagesordnungspunkte['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: TagesordnungspunkteDialogDefaults;
...
export function TagesordnungspunkteDialog({ open, onClose, onSubmit, defaultValues, recordId, sitzungenList, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: TagesordnungspunkteDialogProps) {
  const [fields, setFields] = useState<Partial<Tagesordnungspunkte['fields']>>({});
  ...
  const REQUIRED_FIELDS = ['sitzung', 'punkt_titel'] as const;
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<Tagesordnungspunkte['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'tagesordnungspunkte');
      await onSubmit(clean as Tagesordnungspunkte['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "sitzung": string | null, // Display name from Sitzungen (see <available-records>)\n  "punkt_titel": string | null, // Titel des Tagesordnungspunkts\n  "beschreibung": string | null, // Beschreibung\n  "reihenfolge": number | null, // Reihenfolge / Nummer\n  "dauer": number | null, // Geplante Dauer (Minuten)\n  "typ": LookupValue | null, // Typ des Punktes (select one key: "information" | "beschluss" | "diskussion" | "sonstiges") mapping: information=Information, beschluss=Beschluss, diskussion=Diskussion, sonstiges=Sonstiges\n  "referent": string | null, // Display name from Mitglieder (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["sitzung", "referent"]);
        ...
        const referentName = raw['referent'] as string | null;
        if (referentName) {
          const referentMatch = mitgliederList.find(r => matchName(referentName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (referentMatch) merged['referent'] = createRecordUrl(APP_IDS.MITGLIEDER, referentMatch.record_id);
        }
        return merged as Partial<Tagesordnungspunkte['fields']>;
      });
      ...
      if (file && (file.type.startsWith('image/') || file.type === 'application/pdf')) {
        try {
          ...
          setFields(prev => ({ ...prev, unterlagen: fileUrl }));
        } catch (uploadErr) {
  ...
  const DIALOG_INTENT = defaultValues
    ? t('edit_entity', { entity: appLabel('tagesordnungspunkte') })
    : t('new_entity', { entity: appLabel('tagesordnungspunkte') });

  const fieldBlocks: Record<string, React.ReactNode> = {
    'sitzung': (
      <div key="sitzung" className="space-y-1.5">
        <Label htmlFor="sitzung">{fieldLabel('tagesordnungspunkte', 'sitzung')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Combobox
          id="sitzung"
          placeholder="Zu welcher Sitzung gehört dieser Punkt?"
    ...
    'punkt_titel': (
      <div key="punkt_titel" className="space-y-1.5">
        <Label htmlFor="punkt_titel">{fieldLabel('tagesordnungspunkte', 'punkt_titel')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="punkt_titel"
          placeholder="z. B. Bericht des Vorstandes"
          value={fields.punkt_titel ?? ''}
          onChange={e => setFields(f => ({ ...f, punkt_titel: e.target.value }))}
          required
        />
        {showErrors && !fields.punkt_titel && (
          <p className="text-xs text-destructive mt-1">{t('required_hint')}</p>
        )}
      </div>
    ),
    'beschreibung': (
      <div key="beschreibung" className="space-y-1.5">
        <Label htmlFor="beschreibung">{fieldLabel('tagesordnungspunkte', 'beschreibung')}</Label>
        <Textarea
          id="beschreibung"
          placeholder="Details, erwartete Ergebnisse..."
    ...
    'reihenfolge': (
      <div key="reihenfolge" className="space-y-1.5">
        <Label htmlFor="reihenfolge">{fieldLabel('tagesordnungspunkte', 'reihenfolge')}</Label>
        <Input
          id="reihenfolge"
          type="number"
          step="any"
          {...numberInputProps(formEnhancements, 'reihenfolge')}
          placeholder="z. B. 1"
          value={fields.reihenfolge !== undefined ? fields.reihenfolge : (computedValues['reihenfolge'] ?? '')}
          onChange={e => setFields(f => ({ ...f, reihenfolge: clampNumberValue(formEnhancements, 'reihenfolge', e.target.value) }))}
        />
      </div>
    ),
    'dauer': (
      <div key="dauer" className="space-y-1.5">
        <Label htmlFor="dauer">{fieldLabel('tagesordnungspunkte', 'dauer')}</Label>
        <Input
          id="dauer"
          type="number"
          step="any"
    ...
    'typ': (
      <div key="typ" className="space-y-1.5">
        <Label htmlFor="typ">{fieldLabel('tagesordnungspunkte', 'typ')}</Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.typ) === 'information'}
            onClick={() => setFields(f => ({ ...f, typ: (lookupKey(f.typ) === 'information' ? undefined : 'information') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.typ) === 'information'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('tagesordnungspunkte', 'typ', 'information') ?? 'Information'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.typ) === 'beschluss'}
            onClick={() => setFields(f => ({ ...f, typ: (lookupKey(f.typ) === 'beschluss' ? undefined : 'beschluss') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.typ) === 'beschluss'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('tagesordnungspunkte', 'typ', 'beschluss') ?? 'Beschluss'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.typ) === 'diskussion'}
            onClick={() => setFields(f => ({ ...f, typ: (lookupKey(f.typ) === 'diskussion' ? undefined : 'diskussion') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.typ) === 'diskussion'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('tagesordnungspunkte', 'typ', 'diskussion') ?? 'Diskussion'}
          </button>
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.typ) === 'sonstiges'}
            onClick={() => setFields(f => ({ ...f, typ: (lookupKey(f.typ) === 'sonstiges' ? undefined : 'sonstiges') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.typ) === 'sonstiges'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
            {lookupLabel('tagesordnungspunkte', 'typ', 'sonstiges') ?? 'Sonstiges'}
          </button>
        </div>
      </div>
    ),
    'referent': (
      <div key="referent" className="space-y-1.5">
        <Label htmlFor="referent">{fieldLabel('tagesordnungspunkte', 'referent')}</Label>
        <Combobox
          id="referent"
          placeholder="Wer leitet diesen Punkt?"
          items={mitgliederListAll.map(r => ({
            id: r.record_id,
            label: String(r.fields.vorname ?? r.record_id),
          }))}
          value={extractRecordId(fields.referent)}
          onChange={id => setFields(f => ({ ...f, referent: id ? createRecordUrl(APP_IDS.MITGLIEDER, id) : undefined }))}
          onCreateNew={(q) => openCreateMitglieder("referent", q)}
          createLabel={t('create_in', { entity: appLabel('mitglieder') })}
        />
      </div>
    ),
    'unterlagen': (
      <div key="unterlagen" className="space-y-1.5">
        <Label htmlFor="unterlagen">{fieldLabel('tagesordnungspunkte', 'unterlagen')}</Label>
        {fields.unterlagen ? (
          <div className="flex items-center gap-3 rounded-lg border p-2">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm truncate text-foreground">{fields.unterlagen.split("/").pop()}</p>
              <div className="flex gap-2 mt-1">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setFields(f => ({ ...f, unterlagen: fileUrl }));
                ...
                <button
                  ...
                  onClick={() => setFields(f => ({ ...f, unterlagen: undefined }))}
                >
                  {t('fr_remove')}
                </button>
              </div>
          ...
          <div className="flex-1 overflow-y-auto overflow-x-hidden px-6 py-4 space-y-4 min-w-0">
            ...
            {recordId && (
              <div className="pt-2 border-t border-border">
                <AttachmentsSection appId={APP_IDS.TAGESORDNUNGSPUNKTE} recordId={recordId} />
              </div>
            )}
          </div>
          {submitError && (
