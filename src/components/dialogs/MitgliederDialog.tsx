...
export type MitgliederDialogDefaults = Omit<Mitglieder['fields'], 'status'> & {
...
export function MitgliederDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitgliederDialogProps) {
  ...
  const REQUIRED_FIELDS = ['vorname', 'nachname', 'email', 'status'] as const;
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'email': (
      <div key="email" className="space-y-1.5">
        <Label htmlFor="email">{fieldLabel('mitglieder', 'email')} <span className="text-destructive" aria-hidden="true">*</span></Label>
        <Input
          id="email"
          type="email"
    ...
    'funktion': (
      <div key="funktion" className="space-y-1.5">
        <Label htmlFor="funktion">{fieldLabel('mitglieder', 'funktion')}</Label>
        <Input
          id="funktion"
          placeholder="z. B. Vorsitzende"
          value={fields.funktion ?? ''}
          onChange={e => setFields(f => ({ ...f, funktion: e.target.value }))}
        />
      </div>
    ),
    'abteilung': (
      <div key="abteilung" className="space-y-1.5">
        <Label htmlFor="abteilung">{fieldLabel('mitglieder', 'abteilung')}</Label>
        <Input
          id="abteilung"
          placeholder="z. B. Bereich Forschung"
    ...
    'eintrittsdatum': (
      <div key="eintrittsdatum" className="space-y-1.5">
        <Label htmlFor="eintrittsdatum">{fieldLabel('mitglieder', 'eintrittsdatum')}</Label>
        <DatePicker
          id="eintrittsdatum"
          placeholder="Wann ist der Eintritt?"
          mode="date"
    ...
    'status': (
      <div key="status" className="space-y-1.5">
        ...
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          ...
          >
            {lookupLabel('mitglieder', 'status', 'inaktiv') ?? 'Inaktiv'}
          </button>
        </div>
    ...
    'profilbild': (
      <div key="profilbild" className="space-y-1.5">
        <Label htmlFor="profilbild">{fieldLabel('mitglieder', 'profilbild')}</Label>
        {fields.profilbild ? (
    ...
    'anmerkungen': (
      <div key="anmerkungen" className="space-y-1.5">
        <Label htmlFor="anmerkungen">{fieldLabel('mitglieder', 'anmerkungen')}</Label>
        <Textarea
          id="anmerkungen"
          placeholder="Besonderheiten, Kontaktdaten..."
  ...
  const FIELD_LABELS: Record<string, string> = {"vorname": "Vorname", "nachname": "Nachname", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "funktion": "Funktion im Gremium", "abteilung": "Abteilung / Organisation", "eintrittsdatum": "Eintrittsdatum", "status": "Status", "profilbild": "Profilbild", "anmerkungen": "Anmerkungen"};
