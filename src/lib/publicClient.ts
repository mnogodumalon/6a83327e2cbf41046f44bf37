...
export interface PublicPagesConfig {
  ...
  /** Owner preview (`?preview=1`): the data calls go through Klar with the
...
/** `#/public/<slug>?preview=1` — the OWNER looking at a page that is still a
 *  draft. Without this the only way to see a page was to publish it, i.e. to
 *  make it world-reachable first and hope it looks right.
 *
 ...
 *  data through Klar with the owner's session instead — same page code, same
 *  field projection, no public exposure. */
export function isPreviewRequest(): boolean {
  const hash = window.location.hash || '';
  const q = hash.indexOf('?');
  if (q === -1) return false;
  return new URLSearchParams(hash.slice(q + 1)).get('preview') === '1';
}
 ...
 * callers render the "unavailable" state for all of those. Pass the page's
 * slug to enable the owner preview (see isPreviewRequest).
 */
export async function loadPublicPagesConfig(slug?: string): Promise<PublicPagesConfig | null> {
  if (slug && isPreviewRequest()) {
    const preview = await loadPreviewConfig(slug);
    if (preview) return preview;
    // fall through: a published page still renders from the artifact
  }
  try {
