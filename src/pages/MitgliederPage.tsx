...
export default function MitgliederPage() {
  ...
  return (
    <PageShell
      ...
      subtitle={`${records.length} ${t('in_system', { entity: appLabel('mitglieder') })}`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder={t('search_entity', { entity: appLabel('mitglieder') })}
          value={search}
          ...
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('nachname')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'nachname')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('funktion')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'funktion')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('abteilung')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'abteilung')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('eintrittsdatum')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'eintrittsdatum')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('profilbild')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'profilbild')}
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('anmerkungen')}>
                <span className="inline-flex items-center gap-1">
                  {fieldLabel('mitglieder', 'anmerkungen')}
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mitglieder/${record.record_id}`); }}>
                ...
                <TableCell>{record.fields.funktion ?? '—'}</TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{lookupLabel('mitglieder', 'status', record.fields.status?.key) ?? record.fields.status?.label ?? '—'}</span></TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={11} className="text-center py-16 text-muted-foreground">
                  {search ? t('no_results') : t('no_data_yet', { entity: appLabel('mitglieder') })}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
