...
import type { Mitglieder, Sitzungen, Tagesordnungspunkte, Protokolle, Feedback, Notizen, CreateMitglieder, CreateSitzungen, CreateTagesordnungspunkte, CreateProtokolle, CreateFeedback, CreateNotizen } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- SITZUNGEN ---
  static async getSitzungen(): Promise<Sitzungen[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.SITZUNGEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Sitzungen[];
    return hydrateRecords(records, 'sitzungen');
  }
  static async getSitzungenEntry(id: string): Promise<Sitzungen | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.SITZUNGEN}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Sitzungen;
    return hydrateRecords([record], 'sitzungen')[0];
  }
  static async createSitzungenEntry(fields: CreateSitzungen): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.SITZUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'sitzungen') });
    return { ...data, record_id: data.id };
  }
  static async updateSitzungenEntry(id: string, fields: Partial<CreateSitzungen>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.SITZUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'sitzungen') });
    return { ...data, record_id: data.id };
  }
  static async deleteSitzungenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.SITZUNGEN}/records/${id}`);
  }

  // --- TAGESORDNUNGSPUNKTE ---
  static async getTagesordnungspunkte(): Promise<Tagesordnungspunkte[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TAGESORDNUNGSPUNKTE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Tagesordnungspunkte[];
    return hydrateRecords(records, 'tagesordnungspunkte');
  }
  static async getTagesordnungspunkteEntry(id: string): Promise<Tagesordnungspunkte | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TAGESORDNUNGSPUNKTE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Tagesordnungspunkte;
    return hydrateRecords([record], 'tagesordnungspunkte')[0];
  }
  static async createTagesordnungspunkteEntry(fields: CreateTagesordnungspunkte): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.TAGESORDNUNGSPUNKTE}/records`, { fields: cleanFieldsForApi(fields as any, 'tagesordnungspunkte') });
    return { ...data, record_id: data.id };
  }
  static async updateTagesordnungspunkteEntry(id: string, fields: Partial<CreateTagesordnungspunkte>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.TAGESORDNUNGSPUNKTE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'tagesordnungspunkte') });
    return { ...data, record_id: data.id };
  }
  static async deleteTagesordnungspunkteEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TAGESORDNUNGSPUNKTE}/records/${id}`);
  }

  // --- PROTOKOLLE ---
  static async getProtokolle(): Promise<Protokolle[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.PROTOKOLLE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Protokolle[];
    return hydrateRecords(records, 'protokolle');
  }
  static async getProtokolleEntry(id: string): Promise<Protokolle | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.PROTOKOLLE}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Protokolle;
    return hydrateRecords([record], 'protokolle')[0];
  }
  static async createProtokolleEntry(fields: CreateProtokolle): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.PROTOKOLLE}/records`, { fields: cleanFieldsForApi(fields as any, 'protokolle') });
    return { ...data, record_id: data.id };
  }
  static async updateProtokolleEntry(id: string, fields: Partial<CreateProtokolle>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.PROTOKOLLE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'protokolle') });
    return { ...data, record_id: data.id };
  }
  static async deleteProtokolleEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.PROTOKOLLE}/records/${id}`);
  }

  // --- FEEDBACK ---
  static async getFeedback(): Promise<Feedback[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.FEEDBACK}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec,
      createdat: rec.created_at ?? '', updatedat: rec.updated_at ?? null,
    })) as Feedback[];
    return hydrateRecords(records, 'feedback');
  }
  static async getFeedbackEntry(id: string): Promise<Feedback | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.FEEDBACK}/records/${id}`);
    const record = { record_id: data.id, ...data, createdat: data.created_at ?? '', updatedat: data.updated_at ?? null } as Feedback;
    return hydrateRecords([record], 'feedback')[0];
  }
  static async createFeedbackEntry(fields: CreateFeedback): Promise<MutationResult> {
    const data = await callApi('POST', `/apps/${APP_IDS.FEEDBACK}/records`, { fields: cleanFieldsForApi(fields as any, 'feedback') });
    return { ...data, record_id: data.id };
  }
  static async updateFeedbackEntry(id: string, fields: Partial<CreateFeedback>): Promise<MutationResult> {
    const data = await callApi('PATCH', `/apps/${APP_IDS.FEEDBACK}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'feedback') });
    return { ...data, record_id: data.id };
  }
  static async deleteFeedbackEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.FEEDBACK}/records/${id}`);
  }
