...
export default function Booking() {
  ...
  useEffect(() => {
    // Pass the slug: it enables the owner's draft PREVIEW (?preview=1).
    // Calling it without the slug silently opts the page out.
...
(its record_id) and needs a `list` endpoint on this SAME page — the page can
